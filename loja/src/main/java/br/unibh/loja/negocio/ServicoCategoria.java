...
public class ServicoCategoria implements DAO<Categoria, Long> {
	...
	@SuppressWarnings("unchecked")
	public List<Categoria> findByName(String categoria) throws Exception {
		log.info("Encontrando o " + categoria);
		return em.createNamedQuery("Categoria.findByName").setParameter("descricao", "%" + categoria + "%").getResultList();
	}
}
